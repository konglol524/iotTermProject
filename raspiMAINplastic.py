...
def detect_objects(img):
    ...
    # Resize to 300x300, normalize pixel values
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > 0.6:  # Increased threshold for better accuracy
            ...
            # [LOGGING] Print what the AI sees to verify it's working
            print(f"[DEBUG] Saw {label_name} with {confidence*100:.1f}% confidence")
            
            # Check if the detected object is a bottle (Index 15)
    ...
    # Only print this once per frame, not per detection
    if not found_anything:
        print("[DEBUG] No objects detected above confidence threshold")
...
def generate_frames():
    ...
    # Variables for Frame Skipping (Latency Fix)
    frame_count = 0
    SKIP_FRAMES = 5  # Run AI only every 5 frames
    last_detections = [] # Store the boxes to draw them on skipped frames
    ...
    while True:
        try:
            ...
            # FIX: Convert 4-channel (BGRA/XRGB) to 3-channel (BGR)
            if frame.shape[2] == 4:
            ...
            # [LOGGING] Heartbeat every 30 frames (~1 sec) to show loop is alive
            if frame_count % 30 == 0:
            ...
            for (label, sX, sY, eX, eY) in last_detections:
                cv2.rectangle(frame, (sX, sY), (eX, eY), (0, 255, 0), 2) # Green for bottle
